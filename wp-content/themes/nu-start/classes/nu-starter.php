...
class NU__Starter
{
	...
	public static function nu__includeSiteSearch(){
		...
		if( ( NU__Starter::$themeSettings['search']['type_of_search'] == 'wp' ) && ( NU__Starter::$themeSettings['search']['enable_site_search'] == true ) ){
			$str = '<div class="nu__sitesearch" tabindex="0">';

			$value = is_search() && !empty( get_search_query() ) ? ' value="'.get_search_query().'"' : '';

			if( is_404() ){
				$value = ' value="'. substr( $_SERVER['REQUEST_URI'], 1) .'"';
			}


			$str .= '<a href="javascript:void(0);" class="nu__sitesearch_toggle"></a>';
			...
			$str .= '
				<div class="nu__sitesearch-form--container">
					<form role="search" method="get" action="'.home_url().'" aria-label="site search form">
						...
						<label for="s">site search input</label>
						<input name="s" id="sitesearch-input" placeholder="Search".'.$value.' />
		...
		else if ( ( NU__Starter::$themeSettings['search']['type_of_search'] == 'gcse' ) && ( NU__Starter::$themeSettings['search']['enable_site_search'] == true ) ){

			$str = '<div class="nu__sitesearch">';
			...
			$str .= '
				<div class="nu__sitesearch-form--container">
					<form role="search" method="get" action="'.home_url('/search').'" aria-label="site search form">
						<button type="submit"><i class="fa-regular fa-magnifying-glass"></i></button>
						<label for="query">site search input</label>
