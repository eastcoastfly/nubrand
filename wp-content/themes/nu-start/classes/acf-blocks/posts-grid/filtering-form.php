...
class FilteringForm extends PostsGrid
{
	...
	public static function _build_form_return_string($fields = ''){
		...
		if( !empty($fields) && $fields['autoselect_posts']['post_type'] == 'nu_events' ){
			...
			$chronological_buttons = sprintf(
				$guides['chronological-buttons'],
				'<input value="upcoming"'.$if_checked_upcoming.' id="upcoming" name="chronological" type="radio"></input><label for="upcoming">Upcoming<i class="fa-regular fa-check"></i></label>',
				'<input value="archived"'.$if_checked_archived.' id="archived" name="chronological" type="radio"></input><label for="archived">Archived<i class="fa-regular fa-check"></i></label>',
			);
	...
	private static function _return_the_taxonomy_filters( $taxonomies ){
		...
		foreach( $taxonomies as $taxonomy ){
			...
			// ? this is the earliest we know if there are any associated terms
			if( !empty( $the_taxonomy_filter_terms ) ){
				...
				$taxonomy_filters_return_string .= sprintf(
					...
					strtolower($taxonomy_object->label),
					$select_elem_contents
				);
			}
